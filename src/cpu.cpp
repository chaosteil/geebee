...
const std::array<std::string, 0x100> CPU::opcode_description_{
  ...
  "LD (a8),A", "POP HL", "LD (C),A", "---", "---", "PUSH HL", "AND d8", "RST 20H", "ADD SP,r8", "JP (HL)", "LD (a16),A", "---", "---", "---", "XOR d8", "RST 28H",
...
void CPU::readInstruction() {
  Byte op = memory_.read(pc_++);
  std::cout << "OP: " << opcode_description_[op] << " - 0x" << std::hex << (int)op << std::endl;
  handleOpcode(op);
}
...
int CPU::load16DataAddress() {
  ...
  bits::set_high(af_, memory_.read(address));
...
int CPU::handleResHigh(int bit, int& reg) {
  bits::set_highbit(reg, bit, false);

  return 8;
}

int CPU::handleResLow(int bit, int& reg) {
  bits::set_lowbit(reg, bit, false);
...
int CPU::handleSetHigh(int bit, int& reg) {
  bits::set_highbit(reg, bit, true);

  return 8;
}

int CPU::handleSetLow(int bit, int& reg) {
  bits::set_lowbit(reg, bit, true);
